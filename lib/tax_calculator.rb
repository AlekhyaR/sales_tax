...
class TaxCalculator
  ...
  def calculate_tax(item)
    ...
    cost_of_an_item_including_tax = item.price + total_sales_tax_on_item
    [cost_of_an_item_including_tax, total_sales_tax_on_item]
  end
  ...
  def calculate_import_duty_tax(item)
    return zero_tax_imposed unless imported_good?
